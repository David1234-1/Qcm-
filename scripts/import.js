...
class ImportManager {
  ...
  async uploadFile() {
    if (!this.selectedFile) {
      if (window.StudyHub && window.StudyHub.NotificationManager) {
        window.StudyHub.NotificationManager.show('Veuillez sélectionner un fichier', 'error');
      }
      return;
    }
    ...
    const fileData = {
      ...
      content: null // Sera rempli après traitement
    };
    ...
    this.simulateProcessing(fileData);

    if (window.StudyHub && window.StudyHub.NotificationManager) {
      window.StudyHub.NotificationManager.show('Fichier importé avec succès !', 'success');
    }
  ...
  simulateProcessing(fileData) {
    // Ajouter à la queue de traitement
    this.processingQueue.push(fileData);
    this.updateProcessingQueue();

    // Simuler le traitement avec une progression
    ...
    const interval = setInterval(() => {
      ...
      if (progress >= 100) {
        ...
        fileData.content = this.generateMockContent(fileData);
        ...
        setTimeout(() => {
          ...
          if (window.StudyHub && window.StudyHub.NotificationManager) {
            window.StudyHub.NotificationManager.show('Traitement terminé ! Contenu généré avec succès.', 'success');
          }
        }, 1000);
      }
    }, 100);
