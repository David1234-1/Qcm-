...
class AIChatManager {
  ...
  async generateAIResponse(userQuestion) {
    ...
    try {
      ...
      const response = await this.processAIResponse(userQuestion, context);
  ...
  async processAIResponse(question, context) {
    // Simulation d'une API IA
    // En production, cela utiliserait OpenAI, HuggingFace ou une API similaire
    return new Promise((resolve) => {
      setTimeout(() => {
        const response = this.generateContextualResponse(question, context);
        resolve(response);
